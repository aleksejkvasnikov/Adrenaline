...
new const RankColors[MAX_RANKS] = {
    0xAAAAAAFF, // Rookie - Ñåðûé
    0xFFFFFFAA, // Street Rider - Áåëûé
    0x55DDFFFF, // Tuner - Ãîëóáîé
    0x0070FFFF, // Drift Kid - Ñèíèé
    0xFFFF00FF, // Highway Hunter - Æ¸ëòûé
    0xFFA500FF, // Turbo Outlaw - Îðàíæåâûé
    0xFF0000FF, // Race King - Êðàñíûé
    0xBB00BBFF, // JDM Legend - Ìàëèíîâûé
    0x8A2BE2FF, // Midnight Master - Ôèîëåòîâûé
    0x000000FF, // Fast & Furious - ×¸ðíûé
    0xFFD700FF  // The Boss - Çîëîòîé
};
...
/*	if (row==0)
	...
	if (Current == voteMenu)
	{
		...
		format(vmsg,256,"* %s ïðîãîëîñîâàë çà %s",pname,gVoteItems[row]);
        ...
        if (inGarageMenu[playerid] == 1) {
            if (gPlayerData[playerid][pBoughtCarsHealth][row] < 25) {
                SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] ìàøèíà íå êóïëåíà èëè ñëîìàíà");
            } else {
            ...
            if (gPlayerData[playerid][pMoney] < carCost[shopCarIds[row]]) {
                SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] íåäîñòàòî÷íî äåíåã");
                TogglePlayerControllable(playerid,1);
                inCarsMenu[playerid] = 0;
                inCarsMenuFinished[playerid] = 1;
            } else if (gPlayerData[playerid][pBoughtCarsHealth][row] > 0) {
                SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] óæå êóïëåíî");
                TogglePlayerControllable(playerid,1);
                inCarsMenu[playerid] = 0;
                inCarsMenuFinished[playerid] = 1;
            } else {
        ...
        if (inGarageMenu[playerid] == 1) {
            if (gPlayerData[playerid][pRentedCarRaces][row] == 0) {
                SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] ìàøèíà íå àðåíäîâàíà");
            } else {
                gPlayerData[playerid][pCurrentCar] = row;
        ...
        if (gPlayerData[playerid][pBoughtCarsHealth][row] == 0) {
            SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] ìàøèíà íå êóïëåíà");
        } else {
    ...
    } else if (Current == buildMenu[xRaceBuilding[playerid]-1])
	{
		...
		if (xCarIds[playerid][0]==0)
		{
		    ...
		    SendClientMessage(playerid, COLOR_TEMP, "Çàäàé íàçâàíèå òðàññû! Eg /set RaceDemon");
...
public OnPlayerText(playerid, text[])
{
    ...
    return 0; // áëîêèðóåì ñòàíäàðòíûé âûâîä
}
...
public OnPlayerKeyStateChange(playerid, KEY:newkeys, KEY:oldkeys)
{
	if(IsKeyJustDown(KEY_FIRE,newkeys,oldkeys) && xRaceBuilding[playerid]>=1)//Has the player just pressed mouse1 when they are in buildmode?
	{
	    ...
	    {
	    		SendClientMessage(playerid,COLOR_TEMP,"Ìàêñèìàëüíîå êîëè÷åñòâî ÷åêïîéíòîâ äîñòèãíóòî!");
	    		return;
	    }
	    ...
	    switch (xPlayerProgress[playerid])
	    {
	        case 0:
	        {
				SendClientMessage(playerid,COLOR_TEMP,"[INFO] Íà÷àëüíàÿ òî÷êà 1 çàäàíà! Òåïåðü èñïîëüçóé FIRE ÷òîáû çàäàòü ñòàðòîâóþ òî÷êó äëÿ èãðîêà 2 (2nd Grid position)");
				new veh = GetPlayerVehicleID(playerid);
				GetVehicleZAngle(veh, xFacing[playerid]);
	        }
	        case 1:
	        {
	        	SendClientMessage(playerid,COLOR_TEMP,"[INFO] Íà÷àëüíàÿ òî÷êà 2 çàäàíà! Òåïåðü èñïîëüçóé FIRE ÷òîáû çàäàòü ãîíî÷íûé ÷åêïîéíò. Íàïèøè /saverace êîãäà çàêîí÷èøü!");
	        }
	        default:
	        {
	            new msg[256];
	            format(msg,256,"[INFO] ×åêïîéíò %d çàäàí! Íàïèøè /saverace êîãäà çàêîí÷èøü",xPlayerProgress[playerid]-2);
...
public OnPlayerSpawn(playerid)
{
    ...
    if (postLoginInited[playerid] == 0) {
        ...
        format(pmsg, sizeof(pmsg), "*** {%06x}[%s]{FFFFFF} %s çàø¸ë íà ñåðâåð", (color >>> 8), RankNames[rank], pname);
    ...
    {
        SendClientMessage(playerid, -1, "{00DDDD}[ÈÍÔÎ]{FFFFFF} Âû íå ãîòîâû ê ãîíêå. Èñïîëüçóéòå êîìàíäó {00FF00}/ready{FFFFFF}, ÷òîáû ïðèñîåäèíèòüñÿ ê ñëåäóþùåé ãîíêå.");
        return 1; // Íå çàïóñêàåì ãîíêó
    }

    // Ãîíêà óæå èä¸ò  ìîæíî äîáðîñèòü íà òðàññó
    if (gTrackTime > 10)
    {
...
public OnPlayerEnterRaceCheckpoint(playerid)
{
	...
	if (gPlayerProgress[playerid]==gMaxCheckpoints-1) //If the player has finished...
	{
		...
		printf("[XP] %s ïîëó÷èë %d XP (âñåãî: %d)  ðàíã: %s", pname, xpReward, gPlayerData[playerid][pXP], RankNames[rank]);
		///
...
dcmd_spawncar(playerid, params[])
{
	if (strlen(params) == 0)
	{
	    SendClientMessage(playerid, -1, "Èñïîëüçîâàíèå: /spawncar [ModelID]");
	    return 1;
	}
	...
	{
	    SendClientMessage(playerid, -1, "Îøèáêà: ID ìàøèíû äîëæåí áûòü îò 400 äî 611.");
	    return 1;
	}
	...
	{
	    // Óñòàíàâëèâàåì èíòåðüåð è âèðòóàëüíûé ìèð, ÷òîáû ìàøèíà áûëà âèäèìîé
	    LinkVehicleToInterior(vehicleid, GetPlayerInterior(playerid));
	    SetVehicleVirtualWorld(vehicleid, GetPlayerVirtualWorld(playerid));

	    new msg[64];
	    format(msg, sizeof(msg), "Ìàøèíà ID %d çàñïàâíåíà.", modelid);
	    SendClientMessage(playerid, 0x00FF00FF, msg);
	}
	else
	{
	    SendClientMessage(playerid, -1, "Îøèáêà ïðè ñîçäàíèè ìàøèíû.");
	}
...
{
	if (strlen(params) == 0){
	    SendClientMessage(playerid, -1, "Èñïîëüçîâàíèå: /stadium [1-6]");
	    return 1;
	}
	...
	switch (id)
	{
	    ...
	    default: {
	        SendClientMessage(playerid, -1, "Îøèáêà: ââåäèòå ÷èñëî îò 1 äî 6.");
	        return 1;
	    }
	}
	...
	new msg[128];
	format(msg, sizeof(msg), "Âû áûëè òåëåïîðòèðîâàíû íà ñòàäèîí #%d (èíòåðüåð %d)", id, interior);
	SendClientMessage(playerid, 0x00FF00FF, msg);
	return 1;
}
...
dcmd_track(playerid, params[])  //Admin command to force a change to the specified track Usage: /track TRACKNAME
{
	...
	{
		SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] Èçâèíèòå, òîëüêî àäìèí ìîæåò ìåíÿòü òðàññó");
		return 1;
	}
    if  (gTrackTime<=10)
    {
		SendClientMessage(playerid,COLOR_TEMP,"Ñëèøêîì ïîçäíî, óæå èäåò çàìåíà òðàññû");
		return 1;
	}
	new trackname[256];
	if (!sscanf(params, "s", trackname)) SendClientMessage(playerid, COLOR_TEMP, "Èñïîëüçîâàíèå: /track [trackname]");
    new tmp[256],msg[256];
    format(tmp,256,"%s.race",trackname);
	if(!fexist(tmp))
	{
	    SendClientMessage(playerid, COLOR_TEMP,"404: Òðàññà íå ñóùåñòâóåò");
	    return 1;
	}
	format(msg,256,"Àäìèí èçìåíèë òðàññó íà: %s",trackname);
	printf("Ìåíÿåì òðàññó íà %s",trackname);
...
dcmd_ready(playerid, params[])
{
   #pragma unused params
    ...
    if (gPlayerData[playerid][pReady])
    {
		SendClientMessage(playerid, -1, "{00DDDD}[ÑÒÀÒÓÑ]{FFFFFF} Âû îòìå÷åíû êàê {00FF00}ÃÎÒÎÂÛÉ{FFFFFF} äëÿ ñëåäóþùåé ãîíêè.");
        ...
        // Åñëè ýòî ïåðâûé ãîòîâûé èãðîê, çàïóñòèòü ãîíêó
        new readyCount = 0;
        for (new i = 0; i < MAX_PLAYERS; i++)
        {
        ...
        if (gTrackTime > 10) // íèêòî áîëüøå íå ãîòîâ è ãîíêà åù¸ íå èä¸ò
        {
			gTotalRacers++;
			new tmp[5];
    ...
    {
		SendClientMessage(playerid, -1, "{00DDDD}[ÑÒÀÒÓÑ]{FFFFFF} Âû îòìå÷åíû êàê {FF0000}ÍÅ ÃÎÒÎÂÛÉ{FFFFFF}.");
...
dcmd_addtorotation(playerid, params[]) //Admin command to add a user-made track to the rotation list Usage: /addtorotation TRACKNAME
{
	if(!IsPlayerAdmin(playerid))
	{
		SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] Òîëüêî àäìèí ìîæåò âíîñèòü òðàññû â òðåêëèñò");
		return 1;
	}
	new trackname[128];
	if (!sscanf(params, "s", trackname)) SendClientMessage(playerid, COLOR_TEMP, "[ÎØÈÁÊÀ] Èñïîëüçîâàíèå: /track [trackname]");
    new tmp[256];
    format(tmp,256,"%s.race",trackname);
	if(!fexist(tmp))
	{
	    SendClientMessage(playerid, COLOR_TEMP,"[ÎØÈÁÊÀ] 404: Òðàññå íå ñóùåñòâóåò");
	    return 1;
	}
	...
	if(fexist("racenames.txt"))
	{
		...
		fwrite(f,tmp);
		fclose(f);
		format(msg,256,"[ÓÑÏÅØÍÎ] Äîáàâèë %s â ðîòàöèþ",trackname);
		SendClientMessage(playerid,COLOR_TEMP,msg);
	}
...
dcmd_moretime(playerid, const params[])//Admin command to add 35 seconds onto the current track time
{
	if(!IsPlayerAdmin(playerid))
	{
		SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] Èçâèíèòå, òîëüêî àäìèí ìîæåò äîáàâëÿòü âðåìÿ â òðàññó");
		return 1;
	}
    #pragma unused params
    #pragma unused playerid
...
dcmd_rescueme(playerid, const params[]) //Returns you to the start of the race
{
    #pragma unused params
	new State = GetPlayerState(playerid);
	SendClientMessage(playerid, COLOR_TEMP, "Îòïðàâëÿþ òåáÿ íà ñòàðò.");
	if (State!=1)
	{
...
dcmd_pause(playerid, const params[])  //Admin command to pause the countdown to the next track (note: does not stop players, simply use if you need to extend the time for the track)
{
	if(!IsPlayerAdmin(playerid))
	{
		SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] Òîëüêî àäìèí ìîæåò îñòàíàâëèâàòü è ïðîäîëæàòü òðàññó");
		return 1;
	}
    #pragma unused params
	KillTimer(gEndRacetimer);
	SendClientMessageToAll(COLOR_TEMP,"[ÈÍÔÎ] Àäìèí ïîñòàâèë òðàññó íà ïàóçó!");
	SendClientMessage(playerid,COLOR_TEMP,"[ÈÍÔÎ] Òðåê îñòàíîâëåí, ÷òîáû ïðîäîëæèòü /unpause");
 	return 1;
}
dcmd_unpause(playerid, const params[])
{
	...
	SendClientMessageToAll(COLOR_TEMP,"[ÈÍÔÎ] Àäìèí ñíÿë ïàóçó ñ òåêóùåé òðàññû!");
	SendClientMessage(playerid,COLOR_TEMP,"[ÈÍÔÎ] Ïàóçà ñíÿòà");
 	return 1;
}
dcmd_newcar(playerid, const params[]) //Gives the player a replacement car
{
    ...
    {
        SendClientMessage(playerid, COLOR_TEMP, "[ÎØÈÁÊÀ] Âû óæå èçðàñõîäîâàëè ñâîè çàïàñíûå àâòîìîáèëè ýòîé ãîíêè");
        return 1;
    }
	newCar(playerid);
	...
	{
		new msg[256];
		format(msg,256,"[ÈÍÔÎ] Âûäàåì çàïàñíîé àâòîìîáèëü. Òåáå äîñòóïíî %d çàïàñíûõ àâòîìîáèëåé çà ãîíêó.",SPARE_CARS);
		SendClientMessage(playerid, COLOR_TEMP, msg);
		newcar[playerid]++;
	}
 	return 1;
...
{
    #pragma unused params
    SendClientMessage(playerid,COLOR_TEMP,"Äîñòóïíûå êîìàíäû: '/newcar' '/nc' '/rescueme' '/ready'");
	if(IsPlayerAdmin(playerid))SendClientMessage(playerid,COLOR_TEMP,"Àäìèí êîìàíäû: '/pause' '/resume' '/track [racename]' '/addmodetorotation [racename]'");
 	return 1;
}
dcmd_buildhelp(playerid, const params[])
{
    #pragma unused params
	SendClientMessage(playerid,COLOR_TEMP,"Äîñòóïíûå êîìàíäû: '/buildmode on' '/buildmode off' '/set [racename]' '/newcar'");
 	return 1;
}
...
dcmd_buildmode(playerid, params[])
{
	...
	{
		SendClientMessage(playerid, COLOR_TEMP, "Èñïîëüçîâàíèå: /buildmode [on/off]");
		return 1;
	}
	if (strcmp(var1, "on", true)==0)
	{
		...
		SendClientMessage(playerid,COLOR_TEMP,"Çàõîäèì â ðåæèì ñòðîèòåëüñòâà!");
	...
	if (strcmp(var1, "off", true)==0)
	{
		...
		SendClientMessage(playerid,COLOR_TEMP,"Âûõîäèì èç ðåæèìà ñòðîèòåëüñòâà!");
		TogglePlayerControllable(playerid,1);
...
dcmd_set(playerid, params[])
{
	...
	if (xRaceBuilding[playerid]>0)
	{
	        ...
	        if (!sscanf(params, "s", xRaceName[playerid])) SendClientMessage(playerid, COLOR_TEMP, "[ÎØÈÁÊÀ] Èñïîëüçîâàíèå: /set [value]");
	        //modename
			new trackname[256];
			format(trackname,256,"%s.race",xRaceName[playerid]);
			if (fexist(trackname))
			{
				SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] Ýòîò òðåê óæå ñóùåñòâóåò, âûáåðè äðóãîé.");
				return 1;
			} else if (CheckSafeInput(xRaceName[playerid])==0)
			{
				SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] Îøèáêà: Èñïîëüçóé òîëüêî a-z,A-Z,-,_,0-9");
				return 1;
			}
			xRaceBuilding[playerid]++;
			new msg[256];
			...
			SendClientMessage(playerid,COLOR_TEMP,msg);
			TogglePlayerControllable(playerid,1);
			SendClientMessage(playerid,COLOR_TEMP,"[ÈÍÔÎ] Èñïîëüçóé FIRE äëÿ çàäàíèÿ ñòàðòîâî ïîçèöèè äëÿ èãðîêà 1 (1àÿ ïîçèöèÿ â ðåøåòêå)");
	}
 	return 1;
}
...
dcmd_saverace(playerid, const params[])
{
	...
	if(xRaceBuilding[playerid]>0 && xPlayerProgress[playerid]>3)
	{
		...
		GetPlayerName(playerid,pname,MAX_PLAYER_NAME);
		format (msg,256,"%s ñîçäàë òðàññó: %s",pname,xRaceName[playerid]);
		SendClientMessageToAll(COLOR_TEMP,msg);
		xCarIds[playerid][0]=0;
	} else {
	    SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] Íåäîñòàòî÷íî èíôîðìàöèè!");
	}
 	return 1;
}
//////////////////////////////////////////////////
...
public countVotes()
{
	...
	if (votes > 0)
	{
	    ...
	    format(vmsg, sizeof(vmsg), "{00DDDD}[ÃÎÍÊÀ]{FFFFFF} Ãîëîñîâàíèå çàâåðøåíî! {FFD700}%s{FFFFFF} ïîáåæäàåò ñ {FF5500}%d{FFFFFF} ãîëîñàìè. Ïåðåõîä ÷åðåç 10 ñåêóíä...", gVoteItems[equalvotes[rand]], gVotes[index]);
	...
	else
	{
	    ...
	    format(vmsg, sizeof(vmsg), "{00DDDD}[ÃÎÍÊÀ]{FFFFFF} Ãîëîñîâàíèå çàâåðøåíî! Ãîëîñîâ íå áûëî. Ñëó÷àéíî âûáðàíà òðàññà: {FFD700}%s{FFFFFF}. Ïåðåõîä ÷åðåç 10 ñåêóíä...", gVoteItems[rand]);
	    SendClientMessageToAll(-1, vmsg);
	}
...
public RemovePlayersFromVehicles()
{
	for (new i = 0; i < MAX_PLAYERS; i++)
	{
		...
		{
			RemovePlayerFromVehicle(i);
			// Ãîòîâûé èãðîê  ïðîñòî çàìîðàæèâàåì åãî ìàøèíó
...
public changetrack()
{
	printf("Ìåíÿþ òðåê:%s",gNextTrack);
	invalidateResult = 0;
...
CreateCarShop() {    
    ...
    Create3DTextLabel("Ïîêóïêà è àðåíäà ìàøèí", 0x008080FF, 1.54296, 3414, 5.29753, 40.0, 0, false);
    Create3DTextLabel("Ãàðàæ", 0x008080FF, 2.54296, 3414, 5.29753, 40.0, 0, false);
...
AddPlayersToRace(num)
{
    ...
    format(tmp, sizeof(tmp), "/%d", gTotalRacers);
    TextDrawSetString(Ttotalracers, tmp);
    // Ïîêàçàòü èíòåðôåéñ òîëüêî ó ready èãðîêîâ
    for (new i = 0; i < MAX_PLAYERS; i++)
    {
        if (IsPlayerConnected(i) && gPlayerData[i][pReady])
        {
...
public AddRacers(num)
{
	...
	for(new i=num-1;i>-1;i--)
	{
		...
		if (IsPlayerConnected(j) && gGrided[j]==0 && xRaceBuilding[j]==0 && spawned[j]==1 && gPlayerData[j][pReady])
		{
			...
			printf("Ïîìåùàþ %d â ìàøèíó %d ÷åðåç 3 ñåê..",j,pos);
...
CurrentCar(playerid) {
    ...
    if (c < 0 || gPlayerData[playerid][pCurrentRent] == 0 && gPlayerData[playerid][pBoughtCarsHealth][c] < 25
        ...
        SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] Îòñóòñòâóþò äîñòóïíûå òðàíñïîðòíûå ñðåäñòâà.");
        SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] Êóïèòå èëè àðåíäóéòå òðàíñïîðòíîå ñðåäñòâî â /shop, çàòåì âûáåðèòå åãî â ìåíþ /garage, ëèáî âûïîëíèòå ðåìîíò â ìåíþ /garage.");
        SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] Ïåðåêëþ÷åíèå íà ñòàíäàðòíóþ áåñïëàòíóþ ìàøèíó...");
        return 500;
    } else if (gPlayerData[playerid][pCurrentRent] == 0 && gPlayerData[playerid][pBoughtCarsHealth][c] < 44) {
        SendClientMessage(playerid,COLOR_TEMP,"[ÈÍÔÎ] Ó òðàíñïîðòíîãî ñðåäñòâà çàêàí÷èâàåòñÿ çäîðîâüå.");
        SendClientMessage(playerid,COLOR_TEMP,"[ÈÍÔÎ] Êóïèòå èëè àðåíäóéòå òðàíñïîðòíîå ñðåäñòâî â /shop, çàòåì âûáåðèòå åãî â ìåíþ /garage, ëèáî âûïîëíèòå ðåìîíò â ìåíþ /garage.");
    } else if (gPlayerData[playerid][pCurrentRent] == 1 && gPlayerData[playerid][pRentedCarRaces][c] < 2)
        SendClientMessage(playerid,COLOR_TEMP,"[ÈÍÔÎ] Çàêàí÷èâàþòñÿ àðåíäîâàííûå òðàíñïîðòíûå ñðåäñòâà. Êóïèòå èëè àðåíäóéòå òðàíñïîðòíîå ñðåäñòâî â /shop, çàòåì âûáåðèòå åãî â ìåíþ /garage.");
    else if (c == 1) {
        new maxRank = -1;
        ...
        for (new i = 0; i < MAX_PLAYERS; i++)
        {
            ...
            if (differentRanks == 1 && gPlayerData[playerid][pRank] == maxRank) {
                SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] Âûïîëíÿåòñÿ óðàâíèâàíèå. Ñëèøêîì õîðîøåå òðàíñïîðòíîå ñðåäñòâî, âûáåðèòå äðóãîå.");
                SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] Êóïèòå èëè àðåíäóéòå òðàíñïîðòíîå ñðåäñòâî â /shop, çàòåì âûáåðèòå åãî â ìåíþ /garage, ëèáî âûïîëíèòå ðåìîíò â ìåíþ /garage.");
                SendClientMessage(playerid,COLOR_TEMP,"[ÎØÈÁÊÀ] Ïåðåêëþ÷åíèå íà ñòàíäàðòíóþ áåñïëàòíóþ ìàøèíó...");
                return 500;
            }
...
stock ShowXPText(playerid, amount)
{
    ...
    new text[32];
    format(text, sizeof(text), "~g~+%d XP", amount); // Çåë¸íûé öâåò

    TXP[playerid] = TextDrawCreate(320.0, 180.0, text); // ïî öåíòðó
    TextDrawFont(TXP[playerid], 3);  // êðóïíûé øðèôò
    TextDrawLetterSize(TXP[playerid], 1.0, 3.0);
    TextDrawColor(TXP[playerid], 0x00FF00FF); // ÿðêî-çåë¸íûé
...
SaveRace(playerid)
{
	...
	fclose(f);
	new msgx[256];
	format(msgx,256,"Óñïåøíî! %s ñîçäàíà. Íàïèøè /track %s äëÿ èçìåíåíèÿ",xRaceName[playerid], xRaceName[playerid]);
	SendClientMessage(playerid,COLOR_TEMP,msgx);
	printf("ÑÎÕÐÀÍÅÍÎ %s",xRaceName[playerid]);
	return 1;
}
...
LoadRace(racename[])
{
	printf("Çàãðóçêà òðàññû:%s",racename);
	new trackname[256];
	format (trackname,256,"%s.race",racename);
	if(!fexist(trackname))
	{
	    printf("404:òðàññà íå íàéäåíà: %s",trackname);
	...
	// Ïåðâîå ñîîáùåíèå  çàãîëîâîê òðàññû è àâòîðà
	format(gBestTime, sizeof(gBestTime),
		"{FFD700}<>{FFFFFF} Òðàññà: {FFD700}%s{FFFFFF} | Àâòîð: {00FF00}%s{FFFFFF}",
		gTrackName, gRaceMaker);
	SendClientMessageToAll(-1, gBestTime);
	
	// Âòîðîå ñîîáùåíèå  ðåêîðä
	SendClientMessageToAll(-1, tempTime);
	print("Ãîíêà çàãðóæåíà");
	return 1;
}
...
public nextRaceCountdown()
{
	...
	switch (gTrackTime)
	{
	    ...
	    {
			new msg[128];
			format(msg, sizeof(msg), "{00DDDD}[ÈÍÔÎ]{FFFFFF} Äî ñòàðòà ñëåäóþùåé ãîíêè îñòàëîñü {FFD700}%d {FFFFFF}ñåêóíä.", gTrackTime);
...
public OnPlayerConnect(playerid)
{
    ...
    TogglePlayerSpectating(playerid, true); // âàæíî
    ...
    if (fexist(path)) {
        ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_INPUT, "Àâòîðèçàöèÿ", "Ââåäèòå ïàðîëü:", "Âîéòè", "Âûõîä");
    } else {
        ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_INPUT, "Ðåãèñòðàöèÿ", "Ïàðîëü:", "Îê", "Âûõîä");
    }
...
{   
    ...
    format(pmsg, sizeof(pmsg), "*** {%06x}[%s]{FFFFFF} %s ïîêèíóë ñåðâåð", (color >>> 8), RankNames[rank], pname);
    SendClientMessageToAll(COLOR_TEMP, pmsg);
#endif
...
public OnPlayerDeath(playerid, killerid, WEAPON:reason)
{
	...
	//spawned[playerid]=0;
	TogglePlayerSpectating(playerid, true); // çàìîðàæèâàåì ýêðàí
    SetTimerEx("RespawnPlayer", 3000, false, "d", playerid); // àâòîñïàâí ÷åðåç 2 ñåêóíäû
/*	while (gPlayerProgress[playerid]>0)
	{
...
{
    if (!response) return Kick(playerid); // Íàæàë "Âûõîä"
    ...
    if (dialogid == DIALOG_LOGIN)
    {
        ...
        if (SimpleHash(inputtext) == strval(val))
        {
            ...
            printf("[DEBUG] Àâòîðèçàöèÿ óñïåøíà");
            gPlayerData[playerid][pPassword] = SimpleHash(inputtext);
            gPlayerData[playerid][pLoggedIn] = 1;

            LoadPlayerData(playerid); // âêëþ÷àåò SetSpawnInfo è SpawnPlayer

            SendClientMessage(playerid, -1, "{00FF00}[ÓÑÏÅÕ]{FFFFFF} Àâòîðèçàöèÿ óñïåøíà!");
        }
        else
        {
            format(debugResult, sizeof(debugResult), "FAIL");
            printf("[DEBUG] Íåâåðíûé ïàðîëü");

            ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_INPUT,
                             "Àâòîðèçàöèÿ", "Íåâåðíûé ïàðîëü. Ïîâòîðèòå ïîïûòêó:", "Âîéòè", "Âûõîä");
        }
...
stock InitPlayerInterface(playerid)
{
    ...
    //SendClientMessage(playerid, COLOR_TEMP, "Êîìàíäû: '/newcar' '/rescueme' '/ready'");
	...
	format(msg3, sizeof(msg3), "{00DDDD}Êîìàíäû:{FFFFFF} {00FF00}/newcar{FFFFFF}, {00FF00}/rescueme{FFFFFF}, {00FF00}/ready");
    ...
    // Ïîíèæåííîå ïîëîæåíèå (Y = 120.0 âìåñòî 80.0)
    ...
    TextDrawColor(TRankHUD[playerid], color); // öâåò ïî ðàíãó
...
{       //READ and DISPLAY Highscorelist
	...
	if(fexist(FiPo))
	{
		...
		if (display)
		{
			format(himsg, sizeof(himsg),"Ñïèñîê ëèäåðîâ äëÿ %s\n", track);
		...
		for(new i = 0; i <= sizeof(HSList)-1; i++)
		{
			...
			if (display)
			{
				...
				if (Minutes != 50)	format(himsg, sizeof(himsg), "{00DDDD}[ÐÅÊÎÐÄ]{FFFFFF} #%d  {FFD700}%d:%s.%s{FFFFFF}, óñòàíîâëåí {00FF00}%s{FFFFFF}\n", i+1, Minutes, sSeconds, sMSeconds, HSList[i][rRacer]);
	...
	format(himsg, sizeof(himsg),"Òðåê '%s' íå ñóùåñòâóåò.\n", track);
...
CheckAgainstHighScore(playerid, time)
{
	...
	//if no previous record, set dummy record
		...
		{               //new record
			...
			if (i+1 == 1) printf("ÍÎÂÛÉ ÐÅÊÎÐÄ - %s", playername2);
			format(newrecordmessage, sizeof(newrecordmessage), "{00DDDD}[ÐÅÊÎÐÄ]{FFFFFF} Íîâûé ðåêîðä #{FFD700}%d{FFFFFF} íà òðàññå {FFD700}%s{FFFFFF} óñòàíîâëåí {00FF00}%s{FFFFFF}\n", i+1, gTrackName, playername2);
	...
	//new highscore
		...
		//write from old record list
			...
			SendClientMessageToAll(COLOR_TEMP, "Îøèáêà: Ôàéëà òðåêà íå ñóùåñòâóåò");
		}
	}
}
...
HighScore(Track)
{
	...
	new racer[64]; // íå íóæåí òàêîé áîëüøîé áóôåð
	new HighScoreString[144]; // ñòðîãî ïî ëèìèòó
	...
	if (recordtime > 0)
	{
		...
		// Öâåòíîé ôîðìàò ñ ìåòêîé [ÐÅÊÎÐÄ]
		format(HighScoreString, sizeof(HighScoreString),
			"{00DDDD}[ÐÅÊÎÐÄ]{FFFFFF} %d:%s.%s  {00FF00}%s{FFFFFF}",
			Minutes, sSeconds, sMSeconds, racer);
	}
	else
	{
		format(HighScoreString, sizeof(HighScoreString),
			"{00DDDD}[ÐÅÊÎÐÄ]{FFFFFF} Íåò òåêóùåãî ðåêîðäà.");
	}
...
stock SimpleHash(const str[])
{
	...
	{
		hash = (hash * 31 + str[i]) % 100000; // ïðîñòàÿ êîìáèíàöèÿ, ÷òîáû áûëè ðàçíûå çíà÷åíèÿ
	}
	return hash;
}
